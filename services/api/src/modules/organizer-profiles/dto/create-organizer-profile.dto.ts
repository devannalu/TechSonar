...
import { IsEmail, IsEnum, IsNotEmpty, IsOptional, IsString, IsUrl, Matches, MaxLength, MinLength } from 'class-validator';
...
export class CreateOrganizerProfileDto {
  ...
  @IsNotEmpty({ message: 'O tipo de perfil organizador é obrigatório' })
  ...
  @ApiPropertyOptional({
    description: 'Slug de identificação único (URL-friendly). Se não fornecido, será gerado automaticamente a partir do nome.',
    example: 'tech-sisters',
    minLength: 3,
  })
  @IsOptional()
